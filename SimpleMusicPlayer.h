...
public:
    SimpleMusicPlayer(int pin);

    void playTune(const int *melody, const int *tempo);
    void waitForTune();
    void stopTune();

    void updateMusic(int elapsedTimeMs);
...
private:
    ...
    const int *currentTempo;
    const int *currentMelody;
    volatile int currentNote;
    int currentNoteElapsedTime;
...
#define WHOLE_NOTE     (4 * QUARTER_NOTE)
